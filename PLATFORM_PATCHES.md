...
| `cognee/api/v1/remember/remember.py` | L105-L168 (new `_data_to_text_async` helper) + L132 (`_add_to_session` swap from `_data_to_text` → `_data_to_text_async`) | New `_data_to_text_async` reads `UploadFile.read()` for file-like inputs before falling back to sync `_data_to_text`. `_add_to_session` now `await`s the async helper instead of calling sync — needed because upstream `_data_to_text(List[UploadFile])` emits placeholder strings like `"[file: data]"` / `"[UploadFile]"` that match `_SESSION_PLACEHOLDER_PREFIXES` and trigger silent skip at L156-L161. | V2-6 root cause: `POST /v1/remember` with `session_id` plus text data uploaded via multipart (cognee-mcp's `_text_upload(data)` path at `cognee-mcp/src/cognee_client.py:101-105,467`) was silently dropped on the server side while `remember()` returned `status="session_stored"` — direct diskcache inspection at `/app/.data_storage/<user>/.cognee_fs_cache/sessions_db/cache.db` showed 0 remember-side entries vs N recall-side-effect entries. The async-read fix preserves the placeholder behaviour for string-only inputs (regression test added). | `e91d1a8b` (V2-6, 2026-05-31) |
| `cognee/tests/unit/api/v1/test_remember_session_uploadfile.py` | new file | 5 pytest cases covering `_data_to_text_async` (uploadfile list, single uploadfile, read-failure fallback, full `_add_to_session` integration with mocked SessionManager, regression for pure string-placeholder skip). | TDD lock for the V2-6 patch behaviour. | `e91d1a8b` (V2-6, 2026-05-31) |
| `cognee-mcp/src/server.py` | L1126-L1175 (recall tool) | Added `scope: str = None` parameter to the MCP `recall` tool signature, docstring updated, and passed through to `cognee_client.recall(scope=...)`. Cognee SDK + HTTP API already accepted the `scope` field (`api/v1/recall/recall.py:323` + `api/v1/recall/routers/get_recall_router.py:41-48`); only the MCP layer wasn't threading it through. | V2-7 unblocks `scope="session"` from callers (notably `tenant-pi-shell` `/cognition` slash handler) so cognee's internal session→graph auto_fallthrough can be disabled per-call. Without this, cross-tenant queries via the slash handler leak via the shared graph. | (V2-7, 2026-05-31 — SHA filled in post-commit) |
| `cognee-mcp/src/cognee_client.py` | L498-L530 (recall method) | Added `scope: Optional[Union[str, List[str]]] = None` parameter; passes through to the HTTP API payload (when `use_api=True`) AND to the in-process `cognee.recall(scope=...)` kwargs. `Union` added to the `typing` import. | V2-7 same rationale — server.py tool and client.py wrapper are paired; both must accept and forward `scope`. | (V2-7, 2026-05-31 — SHA filled in post-commit) |
